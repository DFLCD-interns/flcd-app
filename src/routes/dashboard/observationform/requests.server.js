...
import { createClassRequestDB, getBaseRequestByUuid, createBaseRequestDB, getUserFromSessionDB, getUsersWithAccessLevel, insertIntoTableDB } from "$lib/server/db";
...
async function insertApprovals(request_id, instructor = null, staff, fic, chair = null) {
    ...
    if (fic){ console.log(fic.id); await insertApproval(fic.id); }
...
export async function createObservationRequestServer(session_id, staff_assistant_id, purpose, timeslots) {
    ...
    const br = await createBaseRequestDB(staff_assistant_id, purpose, 2, user.id); // TODO fix this
    ...
    const idk = await insertApprovals(base_req.id, null, null, fic[0], null);
